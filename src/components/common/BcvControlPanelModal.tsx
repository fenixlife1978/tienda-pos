...
export const BcvControlPanelModal: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<'adjust' | 'impact' | 'history' | 'daily'>('adjust');
  ...
  // Histórico operativo diario: agrupa todas las sincronizaciones del mismo día BCV
  // y conserva una sola tasa por fecha, evitando mostrar cada consulta automática de fondo.
  const dailyHistory = useMemo(() => {
    const byDate = new Map<string, (typeof historyList)[number]>();

    for (const item of historyList) {
      const dateKey = item.effectiveDate || item.date.split('T')[0];
      ...
      // Preferir un registro oficial automático y, dentro del mismo origen,
      // el registro más reciente del día.
      const currentOfficial = current.type === 'automatic';
      const itemOfficial = item.type === 'automatic';
      if ((itemOfficial && !currentOfficial) ||
          (itemOfficial === currentOfficial && new Date(item.date).getTime() > new Date(current.date).getTime())) {
        byDate.set(dateKey, item);
      }
    }
      ...
      <div className="bg-white w-full max-w-3xl rounded-2xl shadow-2xl border border-slate-200 overflow-hidden flex flex-col max-h-[92vh] animate-in fade-in zoom-in-95 duration-150">
        ...
        <div className="flex border-b border-slate-200 bg-white px-6">
          ...
          >
            ...
            Histórico Diario ({dailyHistory.length} días)
          </button>
          <button
            type="button"
          ...
          >
            ...
            Registro Técnico ({historyList.length})
          </button>
        </div>
          ...
          {activeTab === 'daily' && (
            <div className="space-y-4">
              <div className="flex flex-wrap items-center justify-between gap-3">
                <div>
                  <h4 className="font-bold text-slate-800 text-sm">Histórico diario de la tasa BCV</h4>
                  <p className="text-[11px] text-slate-500 mt-0.5">
                    Una sola tasa por día BCV. Las consultas automáticas de segundo plano no generan filas adicionales aquí.
                  </p>
                </div>
                <span className="inline-flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg bg-emerald-50 border border-emerald-200 text-emerald-800 text-[10px] font-bold">
                  <ShieldCheck className="w-3.5 h-3.5" />
                  Vista contable diaria
                </span>
              </div>

              <div className="border border-slate-200 rounded-xl overflow-hidden bg-white shadow-2xs max-h-[430px] overflow-y-auto">
                <table className="w-full text-left text-[11px]">
                  <thead className="bg-slate-100/90 sticky top-0 z-10 text-slate-700 font-bold border-b border-slate-200">
                    <tr>
                      ...
                      <th className="py-2.5 px-3">Actualizada</th>
                    </tr>
                  </thead>
                    ...
                    {dailyHistory.length === 0 ? (
                      <tr>
                        <td colSpan={5} className="py-10 text-center text-slate-400">
                          No hay histórico diario disponible. Use «Cargar Histórico Oficial bcv.today» en la pestaña de historial técnico.
                        </td>
                      </tr>
                    ) : (
                      dailyHistory.map(({ date, item }) => (
                        <tr key={date} className="hover:bg-slate-50/80 transition-colors">
                          <td className="py-2.5 px-3 font-semibold text-slate-800">{date}</td>
                          <td className="py-2.5 px-3 text-right font-mono font-black text-emerald-700">{item.rate.toFixed(2)}</td>
                          ...
                          <td className="py-2.5 px-3">
                            <span className={`px-2 py-0.5 rounded-full text-[9px] font-bold $
                              ...
                              {item.type === 'automatic' ? 'BCV / Automática' : 'Manual'}
                            </span>
                          </td>
                          <td className="py-2.5 px-3 text-slate-500">
                            {new Date(item.date).toLocaleTimeString('es-VE', { hour: '2-digit', minute: '2-digit' })}
                          </td>
                        </tr>
                      ))
                    )}
              ...
              <div className="p-3 bg-slate-50 border border-slate-200 rounded-xl text-[10px] text-slate-600 leading-relaxed">
                <strong className="text-slate-800">Importante:</strong> esta vista no elimina ni modifica el registro técnico. El historial técnico conserva las actualizaciones para auditoría; esta sección las consolida visualmente por fecha para facilitar la consulta de la tasa oficial de cada día.
              </div>
            </div>
          )}
